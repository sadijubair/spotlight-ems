                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('excerpt') is-invalid @enderror" name="excerpt" rows="3" 
                                  placeholder="Short description (optional)">{{ old('excerpt', $news->excerpt) }}</textarea>
                        @error('excerpt')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <input type="date" class="form-control @error('publish_date') is-invalid @enderror" 
                               name="publish_date" value="{{ old('publish_date', $news->published_at ? $news->published_at->format('Y-m-d') : '') }}">
                        @error('publish_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-success" onclick="setStatus('published')"><i class="bx bx-check-circle"></i> Publish News</button>
                            <button type="submit" class="btn btn-warning" onclick="setStatus('draft')"><i class="bx bx-save"></i> Save as Draft</button>
                            <button type="submit" class="btn btn-secondary" onclick="setStatus('archived')"><i class="bx bx-archive"></i> Archive News</button>
                            <a href="{{ route('news.show', $news->slug) }}" class="btn btn-outline-info"><i class="bx bx-show"></i> View News</a>
                            <a href="{{ route('news.index') }}" class="btn btn-outline-danger"><i class="bx bx-x"></i> Cancel</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="mb-3 text-center">
                    <img src="{{ Storage::url($news->featured_image) }}" alt="{{ $news->title }}" class="img-fluid rounded" id="currentImage">
                    <p class="mt-2 mb-0 small text-muted">Current Featured Image</p>
                </div>
                @endif
                <div class="border rounded p-3 text-center">
                    <i class="bx bx-image font-50 text-muted"></i>
                    <p class="mb-2 text-muted">{{ $news->featured_image ? 'Change featured image' : 'Upload featured image' }}</p>
